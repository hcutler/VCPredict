...
vc_list = open("../data/munge/vc-list.csv", "r").read().split("\n")
vcl = {}
...
for row in mycsv:
	...
	if not row[3] in vcl or not row[3] in vcc or nvcl[row[3]] < nvcl_thresh: continue
	...
	investor_names_hist[vcc[row[3]]][row[0]] = row[7]
	...
	funding_rounds_hist[row[6]][row[0]].append(row[7])

cmp = {}

fout = open("../data/munge/crunchbase_munged_t1.csv", "w")
...
for row in mycsv:
	ct += 1
	if ct <= 1 or len(row[3]) == 0 or len(row[1]) == 0: continue

	if not row[3] in nvcl or not row[3] in vcc or nvcl[row[3]] < nvcl_thresh: continue
	...
	for i in investor_names.keys():
		if i in cmp[n]["investor_names"]: out += "1,"#str(cmp[n]["investor_names"][i]) + ","
		else: out += "0,"

		if row[0] in investor_names_hist[i] and date_diff(investor_names_hist[i][row[0]], row[7]) > 0: outl += "1,"
		else: outl += "0,"
	...
	for i in investor_countries.keys():
		if i in cmp[n]["investor_countries"]: out += "1,"#str(cmp[n]["investor_countries"][i]) + ","
		else: out += "0,"
	for i in funding_round_types.keys():
		if i in cmp[n]["funding_rounds"]: out += "1,"#str(cmp[n]["funding_rounds"][i]) + ","
		else: out += "0,"

		if not row[0] in funding_rounds_hist[i]: outl += "0,"
		else: outl += str(num_dates_after(funding_rounds_hist[i][row[0]], row[7])) + ","
