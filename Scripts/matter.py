...
fin = open("../Data/Mattermark/backend/raw.csv", "r").read()
fout = open("../Data/Mattermark/Mattermark Companies.csv", "w")
...
for i in range(1, len(split)):
	...
	rows = split[i].split("Has Mobile App")[1].split(bk)[0].split("\r")
	for j in rows[1:-1]:
		if not "," in j: break
		fout.write(nasc(j) + "\n")
